...
WAPs00<-Wifi1%>%dplyr::select(starts_with("WAP"),UniquePosit)%>%
                ...
                summarise_all(funs(max))
#Get a vector of only the WAPs
WAPs<-grep("^WAP",names(Wifi1))
#get mean of respective WAP columns in Wifi1
apply(Wifi1[,WAPs],2,mean)
#
#Wifi2<-Wifi1[1:5000,]
#
####k-NN Model####
...
levels(kNN_Predict_df$UniquePosit)<-c("00","01","02","03","10","11","12","13","20","21","22","23","24")
####++kNN Plots####
#Plotting the UniquePosit with Lat and Long
...
colnames(RF_Predict_df)[1]<-"UniquePosit"
colnames(RF_Predict_df)[2]<-"LATITUDE"
colnames(RF_Predict_df)[3]<-"LONGITUDE"
RF_Predict_df$UniquePosit<-as.factor(RF_Predict_df$UniquePosit)
levels(RF_Predict_df$UniquePosit)<-c("00","01","02","03","10","11","12","13","20","21","22","23","24")
####++RF Plots####
#Plotting the UniquePosit with Lat and Long
...
ggplot(data=RF_Predict_df,aes(x = LONGITUDE,y = LATITUDE))+
  geom_point(size=1,color="blue")
#Create new data frame to compare the prediction with original and validation data
RF_Predict_df1<-RF_Predict_df
RF_Predict_df1$Allocation<-"predict"
Wifi1$Allocation<-"original"
Wifi_Vald$Allocation<-"validation"
RF_Predict_df1<- rbind(RF_Predict_df1[,1:4],Wifi1[,c(336,327:328,337)],Wifi_Vald[,c(530,521:522,531)])
#Comparable plot of Original, Validation and Prediction Data
ggplot(data=RF_Predict_df1,aes(x = LONGITUDE,y = LATITUDE))+
  geom_point(size=1,aes(colour=Allocation))
####SVM Model####
#________________________________________________SVM_UniquePosit____________________________________________
set.seed(111)
SVM_Fit<-train(UniquePosit~.,
               data = training[,c(WAPs,which(names(training)=="UniquePosit"))],
               method = "svmLinear",
               trControl=kNNControl,
               tuneLength = 2)#,
...
SVM_Predict
postResample(SVM_Predict,testing$UniquePosit)
#Predict on Vald.
SVM_Predict_Vald<-predict(SVM_Fit,newdata = Wifi_Vald)
postResample(SVM_Predict_Vald,Wifi_Vald$UniquePosit)
#________________________________________________SVM_LONGITUDE____________________________________________
set.seed(111)
SVM_Fit_LONGIT<-train(LONGITUDE~.,
               data = training_LONGIT[,c(WAPs,which(names(training)=="LONGITUDE"))],
               method = "svmLinear",
               trControl=kNNControl,
               tuneLength = 3)#,
...
SVM_Fit_LONGIT
#Predict
SVM_Predict_LONGIT<-predict(SVM_Fit_LONGIT,newdata = testing)
postResample(SVM_Predict_LONGIT,testing$UniquePosit)
#Predict on Vald.
SVM_Predict_LONGIT_Vald<-predict(SVM_Fit_LONGIT,newdata = Wifi_Vald)
postResample(SVM_Predict_LONGIT_Vald,Wifi_Vald$UniquePosit)
#________________________________________________SVM_LATITUDE____________________________________________
set.seed(111)
SVM_Fit_LATIT<-train(LATITUDE~.,
                      data = training_LATIT[,c(WAPs,which(names(training)=="LATITUDE"))],
                      method = "svmLinear",
                      trControl=kNNControl,
                      tuneLength = 5,
...
SVM_Fit_LATIT
#Predict
SVM_Predict_LATIT<-predict(SVM_Fit_LATIT,newdata = testing)
postResample(SVM_Predict_LATIT,testing$UniquePosit)
#Predict on Vald.
SVM_Predict_LATIT_Vald<-predict(SVM_Fit_LATIT,newdata = Wifi_Vald)
postResample(SVM_Predict_LATIT_Vald,Wifi_Vald$UniquePosit)
#___________________________________________________________________________________________________________________________
####Predict.vs Vald.####
Predict_vs_Vald<-data.frame(Wifi_Vald$LONGITUDE,Wifi_Vald$LATITUDE,kNN_Predict_Validation_LONGIT,kNN_Predict_Validation_LATIT)
colnames(Predict_vs_Vald)[1:4]<-c("ValdLong","ValdLat","PredLong","PredLat")
Predict_vs_Vald<-Predict_vs_Vald%>%dplyr::mutate(Longvar=ValdLong-PredLong,
                                Latvar=ValdLat-PredLat); Predict_vs_Vald[1:6]<-round(Predict_vs_Vald[1:6],digits = 1)
Predict_vs_Vald$LongVar<-Predict_vs_Vald[1]-Predict_vs_Vald[3]
Predict_vs_Vald$LatVar<-Predict_vs_Vald[2]-Predict_vs_Vald[4]
Predict_vs_Vald$LongVar<-as.numeric(Predict_vs_Vald$LongVar)
Predict_vs_Vald[1:6]<-round(Predict_vs_Vald[1:6],digits = 1)
plot(Predict_vs_Vald$Longvar)
outLong<-boxplot(Predict_vs_Vald$Longvar)$out
